...
async def test_async_transport_maps_provider_statuses_to_typed_errors(
    ...
    with pytest.raises(error_type):
        await _async_client(httpx.MockTransport(handler)).markets.search("EURUSD")
...
async def test_async_transport_retries_safe_requests_and_marks_mutation_outcomes_ambiguous() -> (
    ...
    assert (
        await _async_client(httpx.MockTransport(retry_handler), max_retries=1).markets.search(
            "EURUSD"
        )
    ).items == ()
    assert attempts == 2
    ...
    with pytest.raises(AmbiguousExecutionError):
        await _async_client(httpx.MockTransport(mutation_handler)).positions.create(
            {"epic": "EPIC"}
        )
...
async def test_async_oauth_streaming_session_and_forced_refresh() -> None:
    ...
    assert (await client.markets.search("EURUSD")).items == ()
