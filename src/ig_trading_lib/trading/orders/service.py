...
class OrderService(TradingService):
    ...
    def update_order(self, deal_id: str, order: UpdateWorkingOrder) -> DealReference:
        ...
        try:
            response = requests.put(url, headers=self.headers, json=order.model_dump())
            if response.status_code == 200:
                return DealReference.model_validate(response.json())
            else:
                raise OrderException(
                    "Update working order failed with status code %s: %s"
                    % (response.status_code, response.text)
                )
        except ValidationError as e:
            raise OrderException("Invalid update working order response: %s" % e)
        except requests.RequestException as e:
            raise OrderException("Update working order request failed: %s" % e)
